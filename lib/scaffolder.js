    ...
    path.exists(srcPath, function(exists) {
        if (exists) {
            ...
            mkdirp(targetPath, function(err) {
                ...
                // call ncp to copy the files, providing a empty function as callback if none is defined
                ncp(srcPath, targetPath, callback || function() {});
            });
        }
        else if (callback) {
            callback(new Error('No assets in path: ' + assetPath));
        }
    });
}; //copyAssets
...
Scaffolder.prototype.init = function(appPath, opts) {
    ...
    this.loadPackageData(function(err, packageData) {
        if (! err) {
            ...
            debug('error reading package data: ', err);
        }
